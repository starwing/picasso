...
PIC_API void ps_path_move_to(ps_path* path, const ps_point* p)
{
    if (!picasso::is_valid_system_device()) {
        picasso_status = STATUS_DEVICE_ERROR;
        return;
...
PIC_API void ps_path_line_to(ps_path* path, const ps_point* p)
{
    if (!picasso::is_valid_system_device()) {
        picasso_status = STATUS_DEVICE_ERROR;
        return;
...
PIC_API void ps_path_tangent_arc_to(ps_path* path, float r, const ps_point* tp, const ps_point* ep)
{
    if (!picasso::is_valid_system_device()) {
        picasso_status = STATUS_DEVICE_ERROR;
        return;
...
PIC_API void ps_path_arc_to(ps_path* path, float rx, float ry, float a, int large, int cw, const ps_point* ep)
{
    if (!picasso::is_valid_system_device()) {
        picasso_status = STATUS_DEVICE_ERROR;
        return;
...
PIC_API void ps_path_bezier_to(ps_path* path, const ps_point* cp1, const ps_point* cp2, const ps_point* ep)
{
    if (!picasso::is_valid_system_device()) {
        picasso_status = STATUS_DEVICE_ERROR;
        return;
...
PIC_API ps_path_cmd ps_path_get_vertex(const ps_path* path, unsigned int index, ps_point* point)
{
    if (!picasso::is_valid_system_device()) {
        picasso_status = STATUS_DEVICE_ERROR;
...
PIC_API int ps_path_is_empty(const ps_path* path)
{
    ...
    picasso_status = STATUS_SUCCEED;
    return (!path->path.total_vertices()) ? 1 : 0;
}
...
PIC_API int ps_path_contains(const ps_path* path, const ps_point* p, ps_fill_rule rule)
{
    if (!picasso::is_valid_system_device()) {
        picasso_status = STATUS_DEVICE_ERROR;
        return 0;
    ...
    return picasso::raster_adapter::fill_contents_point(path->path, 
                FLT_TO_SCALAR(p->x), FLT_TO_SCALAR(p->y), (picasso::filling_rule)rule) ? 1 : 0;
}
...
PIC_API void ps_path_add_arc(ps_path* path, const ps_point* cp, float r, float sa, float ea, int cw)
{
    if (!picasso::is_valid_system_device()) {
        picasso_status = STATUS_DEVICE_ERROR;
        return;
...
PIC_API void ps_path_add_rect(ps_path* path, const ps_rect* r)
{
    if (!picasso::is_valid_system_device()) {
        picasso_status = STATUS_DEVICE_ERROR;
        return;
...
PIC_API void ps_path_add_ellipse(ps_path* path, const ps_rect* r)
{
    if (!picasso::is_valid_system_device()) {
        picasso_status = STATUS_DEVICE_ERROR;
        return;
...
PIC_API void ps_path_add_rounded_rect(ps_path*path, const ps_rect* r, float ltx, float lty, float rtx, float rty,
...
PIC_API void ps_path_clipping(ps_path* r, ps_path_operation op, const ps_path* a, const ps_path* b)
{
    if (!picasso::is_valid_system_device()) {
        picasso_status = STATUS_DEVICE_ERROR;
        return;

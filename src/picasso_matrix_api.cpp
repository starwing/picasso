...
PIC_API ps_matrix* ps_matrix_create_init(float xx, float yx, float xy, float yy, float x0, float y0)
{
...
PIC_API void ps_matrix_init(ps_matrix* matrix, float xx, float yx, float xy, 
...
PIC_API void ps_matrix_shear(ps_matrix* matrix, float sx, float sy)
{
    if (!picasso::is_valid_system_device()) {
        picasso_status = STATUS_DEVICE_ERROR;
        return;
...
PIC_API int ps_matrix_is_identity(const ps_matrix* matrix)
{
    ...
    picasso_status = STATUS_SUCCEED;
    return matrix->matrix.is_identity() ? 1 : 0;
}
...
PIC_API void ps_matrix_transform_path(const ps_matrix* matrix, ps_path* path)
{
    ...
    path->path.transform_all_paths(matrix->matrix.impl());
    picasso_status = STATUS_SUCCEED;
}

...
#ifdef _WIN32
#  ifdef PICASSO_DLL
#    if defined(PICASSO_CORE) || defined(PICASSO_LIB)
#       define PIC_API __declspec(dllexport)
#    else
#       define PIC_API __declspec(dllimport)
#    endif
#  endif
#elif defined(__GNUC__) && ((__GNUC__ >= 4) || (__GNUC__ == 3 && __GNUC_MINOR__ >= 3))
#  define PIC_API __attribute__((visibility("default")))
#endif
...
typedef struct _ps_rect {
    float x; /**< locate left of the rectangle. */
    float y; /**< locate top of the rectangle. */
...
typedef struct _ps_color {
    float r; /**< the red component for a color (0 ~ 1). */
    float g; /**< the green component for a color (0 ~ 1). */
    float b; /**< the blue component for a color (0 ~ 1). */
    float a; /**< the alpha component for a color (0 ~ 1). */
} ps_color;
 ...
 * \sa ps_last_status
 */
PIC_API int ps_version(void);

/**
 * \fn int ps_initialize(void)
 * \brief Initialize the picasso drawing environment.
 *
 *  It must be the first to be call.
 *
 * \return  1 if is initialized, otherwise 0.
 *
 * \sa ps_shutdown
 */
PIC_API int ps_initialize(void);
 ...
 * \sa ps_version
 */
PIC_API ps_status ps_last_status(void);
 ...
 */
PIC_API ps_context* ps_context_create(ps_canvas* canvas, ps_context* shared_context);
 ...
 */
PIC_API ps_context* ps_context_ref(ps_context* ctx);
 ...
 */
PIC_API void ps_context_unref(ps_context* ctx);
 ...
 */
PIC_API ps_canvas* ps_context_set_canvas(ps_context* ctx, ps_canvas* canvas);
 ...
 */
PIC_API ps_canvas* ps_context_get_canvas(ps_context* ctx);
 ...
 */
PIC_API ps_canvas* ps_canvas_create(ps_color_format fmt, int width, int height);

/**
 * \fn ps_canvas* ps_canvas_create_with_data(void* data, ps_color_format fmt, int width, int height, int pitch)
 ...
 *                   The size of this memory block should be at least (pitch * height) voids.
 * \param fmt     The Pixel format to use for the canvas.
 * \param width   The width, in pixels, of the required canvas.
 * \param height  The height, in pixels, of the required canvas.
 * \param pitch   The number of voids per row, of the required canvas.
 *
 * \return If the function succeeds, the return value is the pointer to a new canvas object.
 *         If the function fails, the return value is NULL.
 *
 ...
 */
PIC_API ps_canvas* ps_canvas_create_with_data(void* data, ps_color_format fmt,
 ...
 *     ps_canvas_create_from_image, ps_canvas_ref, ps_canvas_unref
 */
PIC_API ps_canvas* ps_canvas_create_compatible(const ps_canvas* canvas,
 ...
 *     ps_canvas_create_from_image, ps_canvas_ref, ps_canvas_unref
 */
PIC_API ps_canvas* ps_canvas_create_from_canvas(ps_canvas* canvas, const ps_rect* rect);
 ...
 */
PIC_API ps_canvas* ps_canvas_create_from_image(ps_image* img, const ps_rect* rect);

/**
 * \fn ps_canvas* ps_canvas_replace_data(ps_canvas* canvas, void* data,
 ...
 */
PIC_API ps_canvas* ps_canvas_replace_data(ps_canvas* canvas, void* data,
                                                 ps_color_format fmt, int width, int height, int pitch);
 ...
 */
PIC_API void ps_canvas_set_mask(ps_canvas* canvas, const ps_mask* mask);
 ...
 */
PIC_API void ps_canvas_reset_mask(ps_canvas* canvas);
 ...
 */
PIC_API void ps_canvas_bitblt(ps_canvas* src, const ps_rect* rect,
                                                ps_canvas* dst, const ps_point* location);
 ...
 */
PIC_API ps_image* ps_image_create(ps_color_format fmt, int width, int height);

/**
 * \fn ps_image* ps_image_create_with_data(void* data, ps_color_format fmt, int width, int height, int pitch)
 ...
 * \param width   The width, in pixels, of the required image.
 * \param height  The height, in pixels, of the required image.
 * \param pitch   The number of voids per row, of the required image.
 *
 * \return If the function succeeds, the return value is the pointer to a new image object.
 *         If the function fails, the return value is NULL.
 *
 ...
 *     ps_image_create_from_data, ps_image_create_from_image, ps_image_ref, ps_image_unref
 */
PIC_API ps_image* ps_image_create_with_data(void* data, ps_color_format fmt,
                                int width, int height, int pitch);
/**
 * \fn ps_image* ps_image_create_from_data(void* data, ps_color_format fmt, int width, int height, int pitch)
 ...
 */
PIC_API ps_image* ps_image_create_from_data(void* data, ps_color_format fmt,
                                int width, int height, int pitch);
/**
 ...
 */
PIC_API ps_image* ps_image_create_compatible(const ps_canvas* canvas, int width, int height);
 ...
 *     ps_image_create_from_data, ps_image_create_from_image, ps_image_ref, ps_image_unref
 */
PIC_API ps_image* ps_image_create_from_canvas(ps_canvas* canvas, const ps_rect* rect);
 ...
 */
PIC_API ps_image* ps_image_create_from_image(ps_image* img, const ps_rect* rect);
...
/**
 * \fn void ps_image_set_allow_transparent(ps_image* img, int allow)
 * \brief Set whether the image allowed be transparent, 0 is default.
 *
 * \param img    Pointer to an existing image object.
 ...
 */
PIC_API void ps_image_set_allow_transparent(ps_image* img, int allow);
 ...
 * \sa ps_image_get_format, ps_image_get_size
 */
PIC_API void ps_image_set_transparent_color(ps_image* img, const ps_color* color);
 ...
 */
PIC_API ps_pattern* ps_pattern_create_image(const ps_image* img, ps_wrap_type x_wrap,
                                                ps_wrap_type y_wrap, const ps_matrix* transform);
 ...
 */
PIC_API void ps_pattern_transform(ps_pattern* pattern, const ps_matrix* matrix);
 ...
 */
PIC_API ps_gradient* ps_gradient_create_linear(ps_gradient_spread spread,
                                                    const ps_point* start, const ps_point* end);
 ...
 */
PIC_API ps_gradient* ps_gradient_create_radial(ps_gradient_spread spread,
                        const ps_point* start, float sradius, const ps_point* end, float eradius);
 ...
 */
PIC_API ps_gradient* ps_gradient_create_conic(ps_gradient_spread spread,
                                    const ps_point* origin, float sangle);
 ...
 */
PIC_API void ps_gradient_transform(ps_gradient* gradient, const ps_matrix* matrix);
 ...
 */
PIC_API ps_gradient* ps_gradient_ref(ps_gradient* gradient);
 ...
 */
PIC_API void ps_gradient_unref(ps_gradient* gradient);
 ...
 */
PIC_API void ps_gradient_add_color_stop(ps_gradient* gradient,
 ...
 */
PIC_API void ps_gradient_clear_color_stops(ps_gradient* gradient);
...
/**
 * \fn ps_mask* ps_mask_create_with_data(void* data, int width, int height)
 * \brief Create a new mask using a given data block.
 *
 * \param data    A pointer to the mask data block in memory.
 *                   Each void of data is a aplha value, from transparent to opaque (0 ~ 255). 
 *                   The size of this memory block should be at least (width * height) voids.
 ...
 */
PIC_API ps_mask* ps_mask_create_with_data(void* data, int width, int height);
 ...
 */
PIC_API void ps_mask_add_color_filter(ps_mask* mask, const ps_color* color);
 ...
 */
PIC_API void ps_set_source_color(ps_context* ctx, const ps_color* color);
 ...
 */
PIC_API void ps_set_source_image(ps_context* ctx, const ps_image* image);
 ...
 */
PIC_API void ps_set_source_pattern(ps_context* ctx, const ps_pattern* pattern);
 ...
 */
PIC_API void ps_set_source_gradient(ps_context* ctx, const ps_gradient* gradient);
 ...
 */
PIC_API void ps_set_source_canvas(ps_context* ctx, const ps_canvas* canvas);
 ...
 */
PIC_API void ps_set_line_cap(ps_context* ctx, ps_line_cap line_cap);
 ...
 *     ps_set_line_dash, ps_reset_line_dash, ps_set_line_inner_join 
 */
PIC_API void ps_set_line_join(ps_context* ctx, ps_line_join line_join);
 ...
 */
PIC_API void ps_set_line_inner_join(ps_context* ctx, ps_line_inner_join line_inner_join);
 ...
 *     ps_set_line_dash, ps_reset_line_dash, ps_set_line_inner_join
 */
PIC_API float ps_set_line_width(ps_context* ctx, float width);
 ...
 *     ps_set_line_dash, ps_reset_line_dash, ps_set_line_inner_join
 */
PIC_API float ps_set_miter_limit(ps_context* ctx, float limit);
 ...
 *     ps_set_line_dash, ps_reset_line_dash, ps_set_line_inner_join
 */
PIC_API void ps_set_stroke_color(ps_context* ctx, const ps_color* color);
 ...
 */
PIC_API void ps_set_line_dash(ps_context* ctx, float start,
 ...
 */
PIC_API void ps_reset_line_dash(ps_context* ctx);
 ...
 */
PIC_API void ps_set_shadow(ps_context* ctx, float x_offset, float y_offset, float blur);
 ...
 */
PIC_API void ps_set_shadow_color(ps_context* ctx, const ps_color* color);
 ...
 */
PIC_API void ps_reset_shadow(ps_context* ctx);
 ...
 */
PIC_API ps_composite ps_set_composite_operator(ps_context* ctx, ps_composite composite);
 ...
 */
PIC_API ps_filter ps_set_filter(ps_context* ctx, ps_filter filter);
 ...
 */
PIC_API ps_fill_rule ps_set_fill_rule(ps_context* ctx, ps_fill_rule rule);
 ...
 */
PIC_API float ps_set_alpha(ps_context* ctx, float alpha);
 ...
 */
PIC_API float ps_set_gamma(ps_context* ctx, float gamma);

/**
 * \fn void ps_set_antialias(ps_context* ctx, int antialias)
 * \brief Set whether the Anti-aliasing should be turn on.
 *
 * \param ctx          Pointer to an existing context object.
 * \param antialias  Boolean value whether the anti-aliasing is turn on. (1 default)
 *
 * \sa ps_set_alpha, ps_set_blur, ps_set_gamma
 */
PIC_API void ps_set_antialias(ps_context* ctx, int antialias);
 ...
 */
PIC_API float ps_set_blur(ps_context* ctx, float blur);
 ...
 */
PIC_API void ps_stroke(ps_context* ctx);
 ...
 */
PIC_API void ps_fill(ps_context* ctx);
 ...
 */
PIC_API void ps_paint(ps_context* ctx);
 ...
 */
PIC_API void ps_clear(ps_context* ctx);
 ...
 */
PIC_API void ps_clip(ps_context* ctx);
 ...
 */
PIC_API void ps_clip_path(ps_context* ctx, const ps_path* path, ps_fill_rule rule);
 ...
 */
PIC_API void ps_clip_rect(ps_context* ctx, const ps_rect* rect);
 ...
 */
PIC_API void ps_clip_device_rect(ps_context* ctx, const ps_rect* rect);
 ...
 */
PIC_API void ps_clip_rects(ps_context* ctx, const ps_rect* rects, unsigned int num_rects);
 ...
 */
PIC_API void ps_reset_clip(ps_context* ctx);
 ...
 * \sa ps_restore
 */
PIC_API void ps_save(ps_context* ctx);
 ...
 * \sa ps_save
 */
PIC_API void ps_restore(ps_context* ctx);
...
/**
 * \fn ps_font* ps_font_create(const char* name, ps_charset charset, float size, int weight, int italic)
 ...
 */
PIC_API ps_font* ps_font_create(const char* name, ps_charset charset,
                                                    float size, int weight, int italic);
 ...
 */
PIC_API ps_font* ps_font_ref(ps_font* font);
 ...
 */
PIC_API void ps_font_unref(ps_font* font);
 ...
 */
PIC_API void ps_font_set_size(ps_font* font, float size);
 ...
 *        ps_font_set_hint, ps_font_set_flip
 */
PIC_API void ps_font_set_weight(ps_font* font, int weight);

/**
 * \fn void ps_font_set_italic(ps_font* font, int italic)
 * \brief Set italic for a font object.
 *
 * \param font    Pointer to an existing font object.
 * \param italic  Whether or not italic for the font. (0 default)
 *
 * \sa ps_font_set_size, ps_font_set_weight, ps_font_set_charset, 
 *        ps_font_set_hint, ps_font_set_flip
 */
PIC_API void ps_font_set_italic(ps_font* font, int italic);
 ...
 *        ps_font_set_hint, ps_font_set_flip
 */
PIC_API void ps_font_set_charset(ps_font* font, ps_charset charset);

/**
 * \fn void ps_font_set_hint(ps_font* font, int hint)
 * \brief Set hiting for a font object.
 *
 * \param font  Pointer to an existing font object.
 * \param hint     Whether or not auto hiting for the font. (1 default)
 *
 * \sa ps_font_set_size, ps_font_set_weight, ps_font_set_italic, 
 *        ps_font_set_charset, ps_font_set_flip
 */
PIC_API void ps_font_set_hint(ps_font* font, int hint);

/**
 * \fn void ps_font_set_flip(ps_font* font, int flip)
 * \brief Set flip for a font object.
 *
 * \param font  Pointer to an existing font object.
 * \param flip     Whether or not flip y for the font. (0 default)
 *
 * \sa ps_font_set_size, ps_font_set_weight, ps_font_set_italic, 
 *        ps_font_set_charset, ps_font_set_hint
 */
PIC_API void ps_font_set_flip(ps_font* font, int flip);
...
/**
 * \fn int ps_get_font_info(ps_context* ctx, ps_font_info* info)
 ...
 * \sa ps_get_font_info
 */
PIC_API ps_font* ps_set_font(ps_context* ctx, const ps_font* font);
 ...
 */
PIC_API ps_size ps_get_text_extent(ps_context* ctx, const void* text, unsigned int length);
 ...
 */
PIC_API void ps_set_text_color(ps_context* ctx, const ps_color * color);
 ...
 */
PIC_API void ps_set_text_stroke_color(ps_context* ctx, const ps_color * color);
 ...
 *     ps_set_text_stroke_color, ps_set_text_render_type, ps_set_text_kerning
 */
PIC_API void ps_text_transform(ps_context* ctx, const ps_matrix* matrix);
 ...
 *     ps_set_text_stroke_color, ps_set_text_render_type, ps_set_text_kerning
 */
PIC_API void ps_set_text_matrix(ps_context* ctx, const ps_matrix* matrix);
...
/**
 * \fn void ps_set_text_antialias(ps_context* ctx, int antialias)
 * \brief Set whether the font allowed be anti-aliasing.
 *
 * \param ctx         Pointer to an existing context object.
 * \param antialias Boolean value whether anti-aliasing is allowed. (1 default)
 *
 * \sa ps_set_text_matrix, ps_set_text_color, ps_set_text_stroke_color, 
 *     ps_set_text_render_type, ps_text_transform, ps_set_text_kerning
 */
PIC_API void ps_set_text_antialias(ps_context* ctx, int antialias);

/**
 * \fn void ps_set_text_kerning(ps_context* ctx, int kerning)
 * \brief Set whether the font auto kerning is allowed.
 *
 * \param ctx         Pointer to an existing context object.
 * \param kerning   Boolean value whether auto kerning is allowed. (1 default)
 *
 * \sa ps_set_text_matrix, ps_set_text_color, ps_set_text_stroke_color, 
 *     ps_set_text_render_type, ps_text_transform, ps_set_text_antialias
 */
PIC_API void ps_set_text_kerning(ps_context* ctx, int kerning);

/**
 * \fn void ps_text_out_length(ps_context* ctx, float x, float y, const char* text, unsigned int length)
 * \brief Draw single void characters (latin-1) at location in user space.
 *
 * \param ctx       Pointer to an existing context object.
 ...
 */
PIC_API void ps_text_out_length(ps_context* ctx, float x, float y,
                                                const char* text, unsigned int length);

/**
 * \fn void ps_wide_text_out_length(ps_context* ctx, float x, float y, const wchar_t* text, unsigned int length)
 ...
 */
PIC_API void ps_wide_text_out_length(ps_context* ctx, float x, float y,
 ...
 */
PIC_API void ps_draw_text(ps_context* ctx, const ps_rect* area, const void* text,
                                unsigned int length, ps_draw_text_type type, ps_text_align align);

/**
 * \fn int ps_get_glyph(ps_context* ctx, int ch, ps_glyph* glyph)
 ...
 */
PIC_API int ps_get_glyph(ps_context* ctx, int ch, ps_glyph* glyph);
 ...
 */
PIC_API void ps_show_glyphs(ps_context* ctx, float x, float y,
                                                    ps_glyph* glyphs, unsigned int length);

/**
 * \fn int ps_get_path_from_glyph(ps_context* ctx, const ps_glyph* glyph, ps_path* path)
 ...
 * \sa ps_show_glyphs
 */
PIC_API int ps_get_path_from_glyph(ps_context* ctx,
 ...
 */
PIC_API ps_size ps_glyph_get_extent(const ps_glyph* glyph);
 ...
 */
PIC_API void ps_translate(ps_context* ctx, float tx, float ty);
 ...
 */
PIC_API void ps_scale(ps_context* ctx, float sx, float sy);
 ...
 */
PIC_API void ps_shear(ps_context* ctx, float shx, float shy);
 ...
 */
PIC_API void ps_rotate(ps_context* ctx, float angle);
 ...
 */
PIC_API void ps_identity(ps_context* ctx);
 ...
 */
PIC_API void ps_transform(ps_context* ctx, const ps_matrix* matrix);
 ...
 * \sa ps_get_matrix
 */
PIC_API void ps_set_matrix(ps_context* ctx, const ps_matrix* matrix);

/**
 * \fn int ps_get_matrix(ps_context* ctx, ps_matrix* matrix)
 ...
 * \sa ps_set_matrix
 */
PIC_API int ps_get_matrix(ps_context* ctx, ps_matrix* matrix);
 ...
 */
PIC_API void ps_world_to_viewport(ps_context* ctx, ps_point* point);
 ...
 */
PIC_API void ps_viewport_to_world(ps_context* ctx, ps_point* point);
 ...
 */
PIC_API ps_matrix* ps_matrix_create(void);
 ...
 */
PIC_API ps_matrix* ps_matrix_create_init(float sx, float shy, float shx,
                                                            float sy, float tx, float ty);
 ...
 */
PIC_API ps_matrix* ps_matrix_create_copy(const ps_matrix* matrix);
 ...
 */
PIC_API ps_matrix* ps_matrix_ref(ps_matrix* matrix);
 ...
 */
PIC_API void ps_matrix_unref(ps_matrix* matrix);
 ...
 */
PIC_API void ps_matrix_init(ps_matrix* matrix, float sx, float shy,
                                            float shx, float sy, float tx, float ty);
 ...
 */
PIC_API void ps_matrix_translate(ps_matrix* matrix, float tx, float ty);
 ...
 */
PIC_API void ps_matrix_scale(ps_matrix* matrix, float sx, float sy);
 ...
 */
PIC_API void ps_matrix_rotate(ps_matrix* matrix, float angle);
 ...
 */
PIC_API void ps_matrix_shear(ps_matrix* matrix, float shx, float shy);
 ...
 */
PIC_API void ps_matrix_invert(ps_matrix* matrix);
 ...
 */
PIC_API void ps_matrix_reset(ps_matrix* matrix);
 ...
 */
PIC_API void ps_matrix_flip_x(ps_matrix* matrix);
 ...
 */
PIC_API void ps_matrix_flip_y(ps_matrix* matrix);
 ...
 */
PIC_API void ps_matrix_multiply(ps_matrix* result, const ps_matrix* a, const ps_matrix* b);

/**
 * \fn int ps_matrix_is_identity(const ps_matrix* matrix)
 * \brief Checks whether the matrix is the identity transform matrix.
 *
 * \param matrix  Pointer to an existing matrix object.
 *
 * \return  1 if is identity, otherwise 0.
 *
 * \sa ps_matrix_is_equal, ps_matrix_get_determinant, ps_matrix_multiply
 */
PIC_API int ps_matrix_is_identity(const ps_matrix* matrix);

/**
 * \fn int ps_matrix_is_equal(const ps_matrix* a, const ps_matrix* b)
 * \brief Checks whether two matrix are equal.
 *
 * \param a  Pointer to an existing matrix object.
 * \param b  Pointer to an existing matrix object.
 *
 * \return  1 if a and b are equal, otherwise 0.
 *
 * \sa ps_matrix_is_identity, ps_matrix_get_determinant, ps_matrix_multiply
 */
PIC_API int ps_matrix_is_equal(const ps_matrix* a, const ps_matrix* b);
 ...
 */
PIC_API float ps_matrix_get_determinant(const ps_matrix* matrix);
 ...
 */
PIC_API void ps_matrix_set_translate_factor(ps_matrix* matrix, float tx, float ty);

/**
 * \fn int ps_matrix_get_translate_factor(ps_matrix* matrix, float *tx, float *ty)
 ...
 */
PIC_API int ps_matrix_get_translate_factor(ps_matrix* matrix, float *tx, float *ty);
 ...
 */
PIC_API void ps_matrix_set_scale_factor(ps_matrix* matrix, float sx, float sy);

/**
 * \fn int ps_matrix_get_scale_factor(ps_matrix* matrix, float *sx, float *sy)
 ...
 */
PIC_API int ps_matrix_get_scale_factor(ps_matrix* matrix, float *sx, float *sy);
 ...
 */
PIC_API void ps_matrix_set_shear_factor(ps_matrix* matrix, float shx, float shy);

/**
 * \fn int ps_matrix_get_shear_factor(ps_matrix* matrix, float *shx, float *shy)
 ...
 */
PIC_API int ps_matrix_get_shear_factor(ps_matrix* matrix, float *shx, float *shy);
 ...
 */
PIC_API void ps_matrix_transform_point(const ps_matrix* matrix, ps_point* point);
 ...
 */
PIC_API void ps_matrix_transform_rect(const ps_matrix* matrix, ps_rect* rect);
 ...
 */
PIC_API void ps_matrix_transform_path(const ps_matrix* matrix, ps_path* path);
 ...
 */
PIC_API void ps_new_path(ps_context* ctx);
 ...
 */
PIC_API void ps_new_sub_path(ps_context* ctx);
 ...
 */
PIC_API void ps_set_path(ps_context* ctx, const ps_path* path);
 ...
 */
PIC_API void ps_close_path(ps_context* ctx);
 ...
 */
PIC_API void ps_move_to(ps_context* ctx, const ps_point* point);
 ...
 *     ps_arc, ps_tangent_arc, ps_rectangle, ps_rounded_rect, ps_ellipse
 */
PIC_API void ps_line_to(ps_context* ctx, const ps_point* point);
 ...
 */
PIC_API void ps_bezier_curve_to(ps_context* ctx, const ps_point* fcp,
                                           const ps_point* scp, const ps_point* ep);
 ...
 *     ps_tangent_arc, ps_rectangle, ps_rounded_rect, ps_ellipse
 */
PIC_API void ps_quad_curve_to(ps_context* ctx, const ps_point* cp, const ps_point* ep);

/**
 * \fn void ps_arc(ps_context* ctx, const ps_point* cp, float radius, 
 *                                        float sangle, float eangle, int clockwise)
 ...
 * \param clockwise  1 is clockwise, 0 is counter clockwise.
 *
 * \sa ps_move_to, ps_line_to, ps_bezier_curve_to, ps_quad_curve_to, 
 *     ps_tangent_arc, ps_rectangle, ps_rounded_rect, ps_ellipse
 */
PIC_API void ps_arc(ps_context* ctx, const ps_point* cp, float radius,
                                           float sangle, float eangle, int clockwise);
 ...
 */
PIC_API void ps_tangent_arc(ps_context* ctx, const ps_rect* rect, float sangle, float sweep);
 ...
 */
PIC_API void ps_rectangle(ps_context* ctx, const ps_rect* rect);
 ...
 */
PIC_API void ps_rounded_rect(ps_context* ctx, const ps_rect* rect, float ltx,
 ...
 */
PIC_API void ps_ellipse(ps_context* ctx, const ps_rect* rect);
 ...
 */
PIC_API ps_path* ps_path_create(void);
 ...
 */
PIC_API ps_path* ps_path_create_copy(const ps_path* path);
 ...
 */
PIC_API ps_path* ps_path_ref(ps_path* path);
 ...
 */
PIC_API void ps_path_unref(ps_path* path);
 ...
 */
PIC_API void ps_path_move_to(ps_path* path, const ps_point* point);
 ...
 *     ps_path_arc_to, ps_path_tangent_arc_to
 */
PIC_API void ps_path_line_to(ps_path* path, const ps_point* point);
 ...
 * \param large_arc  1 is large arc, 0 is small arc.
 * \param clockwise  1 is clockwise, 0 is counter clockwise.
 ...
 */
PIC_API void ps_path_arc_to(ps_path* path, float radiusX, float radiusY, float angle,
                                        int large_arc, int clockwise, const ps_point* ep);
 ...
 *     ps_path_tangent_arc_to, ps_path_line_to
 */
PIC_API void ps_path_bezier_to(ps_path* path, const ps_point* fcp,
                                                 const ps_point* scp, const ps_point* ep);
 ...
 *     ps_path_tangent_arc_to, ps_path_line_to
 */
PIC_API void ps_path_quad_to(ps_path* path, const ps_point* cp, const ps_point* ep);
 ...
 */
PIC_API void ps_path_sub_close(ps_path* path);
 ...
 */
PIC_API float ps_path_get_length(const ps_path* path);
 ...
 */
PIC_API void ps_path_clear(ps_path* path);

/**
 * \fn int ps_path_is_empty(const ps_path* path)
 ...
 */
PIC_API int ps_path_is_empty(const ps_path* path);
 ...
 */
PIC_API unsigned int ps_path_get_vertex_count(const ps_path* path);
 ...
 */
PIC_API ps_path_cmd ps_path_get_vertex(const ps_path* path,
 ...
 */
PIC_API ps_rect ps_path_bounding_rect(const ps_path* path);

/**
 * \fn int ps_path_contains(const ps_path* path, const ps_point* point, ps_fill_rule rule)
 ...
 *
 * \return  1 if the point is contained, otherwise 0.
 *
 * \sa ps_path_get_vertex_count, ps_path_get_vertex, ps_path_bounding_rect
 */
PIC_API int ps_path_contains(const ps_path* path,
 ...
 */
PIC_API void ps_path_add_line(ps_path* path, const ps_point* p1, const ps_point* p2);

/**
 * \fn void ps_path_add_arc(ps_path* path, const ps_point* cp, float radius, float sangle,
 *                                                                float eangle, int clockwise)
 ...
 */
PIC_API void ps_path_add_arc(ps_path* path, const ps_point* cp, float radius,
                                            float sangle, float eangle, int clockwise);
 ...
 */
PIC_API void ps_path_add_rect(ps_path* path, const ps_rect* rect);
 ...
 */
PIC_API void ps_path_add_ellipse(ps_path* path, const ps_rect* rect);
 ...
 */
PIC_API void ps_path_add_rounded_rect(ps_path* path, const ps_rect* rect, float ltx,
                      float lty, float rtx, float rty, float lbx, float lby, float rbx, float rby);
/**
 ...
 */
PIC_API void ps_path_clipping(ps_path* result, ps_path_operation op,

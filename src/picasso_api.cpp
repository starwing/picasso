...
ps_status picasso_status = STATUS_SUCCEED;
...
{
    return (picasso::_init_system_device() && picasso::font_engine::initialize());
}
...
{
    return picasso_status;
}

PIC_API ps_context* ps_context_create(ps_canvas* canvas, ps_context* ctx)
{
...
PIC_API float ps_set_alpha(ps_context* ctx, float a)
{
    if (!picasso::is_valid_system_device()) {
        picasso_status = STATUS_DEVICE_ERROR;
        return 0.0f;
...
PIC_API float ps_set_blur(ps_context* ctx, float b)
{
    if (!picasso::is_valid_system_device()) {
        picasso_status = STATUS_DEVICE_ERROR;
        return 0.0f;
...
PIC_API float ps_set_gamma(ps_context* ctx, float g)
{
    if (!picasso::is_valid_system_device()) {
        picasso_status = STATUS_DEVICE_ERROR;
        return 0.0f;
...
PIC_API void ps_set_antialias(ps_context* ctx, int anti)
{
    if (!picasso::is_valid_system_device()) {
        picasso_status = STATUS_DEVICE_ERROR;
        return;
...
PIC_API void ps_set_shadow(ps_context* ctx, float x, float y, float b)
{
    if (!picasso::is_valid_system_device()) {
        picasso_status = STATUS_DEVICE_ERROR;
        return;
...
PIC_API void ps_set_shadow_color(ps_context* ctx, const ps_color* c)
{
    if (!picasso::is_valid_system_device()) {
        picasso_status = STATUS_DEVICE_ERROR;
        return;
...
PIC_API void ps_set_line_dash(ps_context* ctx, float start, float* dashes, unsigned int num_dashes)
{
    if (!picasso::is_valid_system_device()) {
        picasso_status = STATUS_DEVICE_ERROR;
        return;
...
PIC_API void ps_rectangle(ps_context* ctx, const ps_rect* pr)
{
    if (!picasso::is_valid_system_device()) {
        picasso_status = STATUS_DEVICE_ERROR;
        return;
...
PIC_API void ps_rounded_rect(ps_context* ctx, const ps_rect* r, float ltx, float lty, float rtx, float rty,
...
PIC_API void ps_ellipse(ps_context* ctx, const ps_rect* r)
{
    if (!picasso::is_valid_system_device()) {
        picasso_status = STATUS_DEVICE_ERROR;
        return;
...
PIC_API void ps_move_to(ps_context* ctx, const ps_point* pt)
{
    if (!picasso::is_valid_system_device()) {
        picasso_status = STATUS_DEVICE_ERROR;
        return;
...
PIC_API void ps_line_to(ps_context* ctx, const ps_point* pt)
{
    if (!picasso::is_valid_system_device()) {
        picasso_status = STATUS_DEVICE_ERROR;
        return;
...
PIC_API void ps_arc(ps_context* ctx, const ps_point* cp, float r, 
                                            float sa, float ea, int clockwise)
{
    if (!picasso::is_valid_system_device()) {
        picasso_status = STATUS_DEVICE_ERROR;
        return;
...
PIC_API void ps_tangent_arc(ps_context* ctx, const ps_rect* r, float sa, float sw)
{
    if (!picasso::is_valid_system_device()) {
        picasso_status = STATUS_DEVICE_ERROR;
        return;
...
PIC_API void ps_shear(ps_context* ctx, float sx, float sy)
{
    if (!picasso::is_valid_system_device()) {
        picasso_status = STATUS_DEVICE_ERROR;
        return;
...
PIC_API void ps_clip_path(ps_context* ctx, const ps_path* p, ps_fill_rule r)
{
    if (!picasso::is_valid_system_device()) {
        picasso_status = STATUS_DEVICE_ERROR;
        return;
...
PIC_API void ps_clip_device_rect(ps_context* ctx, const ps_rect* r)
{
    if (!picasso::is_valid_system_device()) {
        picasso_status = STATUS_DEVICE_ERROR;
        return;
...
PIC_API void ps_clip_rect(ps_context* ctx, const ps_rect* r)
{
    if (!picasso::is_valid_system_device()) {
        picasso_status = STATUS_DEVICE_ERROR;
        return;
...
PIC_API void ps_clip_rects(ps_context* ctx, const ps_rect* rs, unsigned int num_rs)
{
    if (!picasso::is_valid_system_device()) {
        picasso_status = STATUS_DEVICE_ERROR;
        return;

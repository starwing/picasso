...
public:
    ...
    typedef struct const_row_info
    {
        ...
        const_row_info(int _x1, int _x2, const void* _ptr)
            : x1(_x1), x2(_x2), ptr((const uint8_t*)_ptr)
        {
        }
    } row_data;

    gfx_rendering_buffer();
    gfx_rendering_buffer(void* ptr, unsigned int width, unsigned int height, int stride);

    virtual void init(void* ptr, unsigned int width, unsigned int height, int stride);
    ...
    virtual void* buffer(void) const { return m_buffer; }
...
public:
    uint8_t* row_ptr(int y) const { return (uint8_t*)m_rows[y]; }
    uint8_t* row_ptr(int, int y, unsigned int) const { return (uint8_t*)m_rows[y]; }
...
private:
    ...
    void* m_buffer;
    pod_array<uint8_t*> m_rows;
    unsigned int m_width; // width in pixels
    unsigned int m_height; // height in pixels
    int m_stride; // number of voids per row, link pitch. can < 0
    // extra property
    bool m_transparent;
    bool m_has_colorkey;
    rgba m_colorkey;
